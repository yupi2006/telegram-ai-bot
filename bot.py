...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        ...
        f"Бесплатно доступно: {FREE_LIMIT} генерации\n"
        f"Осталось: {left}\n\n"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "buy":
        await query.message.reply_text(
            f"⭐ Для доступа напиши @{ADMIN_USERNAME}",
            parse_mode="HTML",
        )
        return

    if query.data in ["gen_token", "gen_meme", "gen_x", "gen_tg", "gen_pack"]:
        if not consume_limit(user_id):
            await query.message.reply_text(
                f"❌ Бесплатный лимит закончился.\n\nНапиши @{ADMIN_USERNAME} для доступа.",
                reply_markup=main_menu(),
                parse_mode="HTML",
            )
            return
